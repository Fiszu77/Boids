...
public class Physics {
    Vector2 boidsVelocity, v1TangentVector, v2TangentVector, v1NormalVector,
            v2NormalVector, v1After, v2After;
    Vector2 unitNormal;
    Vector2 unitTangent;
    float v1NormalBefore, v1Tangent, v2Tangent, v2NormalBefore, v1NormalAfter, v2NormalAfter;

    Physics() {

    }

    public void collideWithObstacle(SimpleBoid boid, Obstacle obstacle, Intersector.MinimumTranslationVector mtv) {
        if (!obstacle.isStartExp()) {
            boidsVelocity = new Vector2(boid.getVelocityVector()).add(boid.getCollisionVelocity());
            unitNormal = new Vector2().set(boid.getLocationVector()).sub(obstacle.getPosition());
            unitNormal.nor();
            unitTangent = new Vector2(-unitNormal.y, unitNormal.x);
            v1NormalBefore = unitNormal.dot(boidsVelocity);
            v1Tangent = unitTangent.dot(boidsVelocity);
            v2NormalBefore = unitNormal.dot(obstacle.getVelocity());
            v2Tangent = unitTangent.dot(obstacle.getVelocity());
            v1NormalAfter = (v1NormalBefore * (boid.getMass() - obstacle.getMass()) + 2 * obstacle.getMass() * v2NormalBefore) / (boid.getMass() + obstacle.getMass());
            v2NormalAfter = (v2NormalBefore * (obstacle.getMass() - boid.getMass()) + 2 * boid.getMass() * v1NormalBefore) / (boid.getMass() + obstacle.getMass());
            v1TangentVector = unitTangent.cpy().scl(v1Tangent);
            v2TangentVector = unitTangent.cpy().scl(v2Tangent);
            v1NormalVector = unitNormal.cpy().scl(v1NormalAfter);
            v2NormalVector = unitNormal.cpy().scl(v2NormalAfter);
            v1After = v1TangentVector.cpy().add(v1NormalVector);
            v2After = v2TangentVector.cpy().add(v2NormalVector);
    ...
    public void collideWithObstacle(MotherBoid boid, Obstacle obstacle, Intersector.MinimumTranslationVector mtv) {
        if (!obstacle.isStartExp()) {
            boidsVelocity = new Vector2(boid.getVelocityVector()).add(boid.getCollisionVelocity());
            unitNormal = new Vector2().set(boid.getLocationVector()).sub(obstacle.getPosition());
            unitNormal.nor();
    ...
    public void collideWithObstacle(Obstacle obstacle1, Obstacle obstacle2, Intersector.MinimumTranslationVector mtv) {
        unitNormal = new Vector2().set(obstacle1.getPosition()).sub(obstacle2.getPosition());
        unitNormal.nor();
        unitTangent = new Vector2(-unitNormal.y, unitNormal.x);
        v1NormalBefore = unitNormal.dot(obstacle1.getVelocity());
        v1Tangent = unitTangent.dot(obstacle1.getVelocity());
        v2NormalBefore = unitNormal.dot(obstacle2.getVelocity());
        v2Tangent = unitTangent.dot(obstacle2.getVelocity());
        v1NormalAfter = (v1NormalBefore * (obstacle1.getMass() - obstacle2.getMass()) + 2 * obstacle2.getMass() * v2NormalBefore) / (obstacle1.getMass() + obstacle2.getMass());
        v2NormalAfter = (v2NormalBefore * (obstacle2.getMass() - obstacle1.getMass()) + 2 * obstacle1.getMass() * v1NormalBefore) / (obstacle1.getMass() + obstacle2.getMass());
        v1TangentVector = unitTangent.cpy().scl(v1Tangent);
        v2TangentVector = unitTangent.cpy().scl(v2Tangent);
        v1NormalVector = unitNormal.cpy().scl(v1NormalAfter);
        v2NormalVector = unitNormal.cpy().scl(v2NormalAfter);
